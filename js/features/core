...
function renderMessages(preserveScroll = false) {
    ...
    msgsToRender.forEach((msg, i) => {
        ...
        // ---- 红包消息特殊渲染 ----
        if (msg.type === 'redpacket' && window.Redpacket && window.Redpacket.renderCard) {
            var bubbleClass = msg.sender === 'user' ? 'message-sent' : 'message-received';
            var rpCard = window.Redpacket.renderCard(msg, bubbleClass);
            ...
            contentWrapper.className = 'message-content-wrapper';
            contentWrapper.appendChild(rpCard);
            rpCard.addEventListener('click', function(e) {
                e.stopPropagation();
                if (window.Redpacket && window.Redpacket.showDetail) {
                    window.Redpacket.showDetail(msg.id);
                }
            });
            wrapper.appendChild(contentWrapper);
            fragment.appendChild(wrapper);
...
const addMessage = (message) => {
    ...
    if (message.type === 'redpacket') {
        ...
        renderMessages();
        throttledSaveData();
        if (window.Redpacket && window.Redpacket.startChecker) {
            window.Redpacket.startChecker();
        }
        return;
    }
    // ---- 红包消息处理结束 ----
