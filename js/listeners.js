...
if (target.classList.contains('delete-btn')) {
    ...
    return;
            ...
            // ---- 红包按钮注入 ----
            (function injectRedpacketButton() {
                ...
                rpBtn.style.cssText = 'background: linear-gradient(135deg, #fce4c8, #f8d5b0); border: none; border-radius: 50%; width: 46px; height: 46px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 2px 8px rgba(230,62,46,0.25); transition: transform 0.2s, box-shadow 0.2s;';
                rpBtn.onmouseover = function() { this.style.transform = 'scale(1.08)'; this.style.boxShadow = '0 4px 16px rgba(230,62,46,0.4)'; };
                rpBtn.onmouseout = function() { this.style.transform = 'scale(1)'; this.style.boxShadow = '0 2px 8px rgba(230,62,46,0.25)'; };
                rpBtn.onclick = function(e) {
                    e.stopPropagation();
                    if (window.Redpacket && typeof window.Redpacket.send === 'function') {
                        window.Redpacket.send();
                    } else {
                        if (typeof showNotification === 'function') showNotification('红包模块未加载，请刷新页面', 'error');
                    }
                };
